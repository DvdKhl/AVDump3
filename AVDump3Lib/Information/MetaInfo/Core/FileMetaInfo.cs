...
public class FileMetaInfo {
	...
	public FileInfo ResolvedPathFileInfo { get; private set; }

	public FileMetaInfo(FileInfo fileInfo, IEnumerable<MetaDataProvider> items) {
		FileInfo = fileInfo;

		var resolvedPathInfo = fileInfo.ResolveLinkTarget(true);
		if (resolvedPathInfo != null) {
			ResolvedPathFileInfo = (FileInfo)resolvedPathInfo;
		} else {
			ResolvedPathFileInfo = fileInfo;
		}
