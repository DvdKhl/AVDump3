...
namespace AVDump3Lib.Processing.BlockConsumers.Matroska {
    public class EBMLBlockDataSource : IEBMLDataSource {
		protected IBlockStreamReader reader;

		//private Int64 absolutePosition;
		protected long relativePosition;
		private long length;
		...
		public Span<byte> GetData(int neededBytes) {
			int blockLength;
		...
		public virtual int ReadIdentifier() {
			int bytesToRead = 0;
			byte mask = 1 << 7;
			int blockLength;
			var block = reader.GetBlock(out blockLength);
