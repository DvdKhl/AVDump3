...
namespace AVDump3Lib.Reporting.Reports {
    public class AniDBReport : XmlReport {
        ...
        public AniDBReport(FileMetaInfo fileMetaInfo) {
            ...
            var mediaProvider = fileMetaInfo.CondensedProviders.OfType<MediaProvider>().Single();
            var avmfElem = new XElement("avmd",
                new XElement("date", ConvertDate(mediaProvider.Select(MediaProvider.CreationDateType)?.Value)),
                new XElement("duration", mediaProvider.Select(MediaProvider.DurationType)),
                new XElement("app", mediaProvider.Select(MediaProvider.MuxingAppType)),
                new XElement("extension", mediaProvider.Select(MediaProvider.SuggestedFileExtensionType)),
                new XElement("file_extension", fileMetaInfo.FileInfo.Extension),
                new XElement("segmient_uid", BitConverter.ToString(mediaProvider.Select(MediaProvider.IdType).Value).Replace("-", "")),
                 new XElement("extra_size", mediaProvider.Select(MediaProvider.OverheadType))
           );
