...
public class AVD3CLModule : AVD3UIModule {
	private AVD3ProgressDisplay progressDisplay = null!;

	public override IAVD3Console Console => console;
	readonly AVD3Console console = new();

	protected override AVD3UISettings Settings => settings;
	private AVD3CLSettings settings = null!;

	public override IBytesReadProgress CreateBytesReadProgress() {
	...
	protected override IStreamProvider CreateFileStream(string[] paths) {
		var sp = base.CreateFileStream(paths);

		switch(sp) {
			case NullStreamProvider nsp:
				progressDisplay.TotalFiles = nsp.StreamCount;
				progressDisplay.TotalBytes = nsp.StreamCount * nsp.StreamLength;
				break;
			case StreamFromPathsProvider sfpp:
				progressDisplay.TotalFiles = sfpp.TotalFileCount;
				progressDisplay.TotalBytes = sfpp.TotalBytes;
				break;
		}
	...
	protected override void OnProcessingStarting(CancellationTokenSource cts) {
		...
		System.Console.CancelKeyPress -= cancelKeyHandler; //Register only once
		System.Console.CancelKeyPress += cancelKeyHandler;
	}
	...
	private void UnhandleException(object sender, UnhandledExceptionEventArgs e) {
		...
		if(e.ExceptionObject is AggregateException aggEx && aggEx.InnerExceptions.Count == 1 && aggEx.InnerExceptions[0] is AVD3ForceMajeureException forceMajeureException) {
			...
			libException = new AVD3UIException("Unhandled AppDomain wide Exception", ex);
		}
	...
	protected override void OnException(AVD3LibException ex) {
		...
		OnExceptionThrown(new AVD3UIModuleExceptionEventArgs(exElem));
...
//		ExceptionThrown?.Invoke(this, new AVD3UIModuleExceptionEventArgs(exElem));
