...
namespace AVDump3CL {
	...
	public class AVD3CLModule : IAVD3CLModule {
		...
		public void Process(string[] paths) {
			...
			var streamConsumerCollection = processingModule.CreateStreamConsumerCollection(
				sp, settings.Processing.BlockSize.BlockCount, settings.Processing.BlockSize.BlockLength
			);
		...
		private async void ConsumingStream(object sender, ConsumingStreamEventArgs e) {
			...
			if(settings.Display.PrintHashes) {
				foreach(var bc in blockConsumers.OfType<HashCalculator>()) {
					cl.Writeline(bc.Name + " => " + BitConverter.ToString(bc.Hash).Replace("-", ""));
				}
				cl.Writeline("");
			}
